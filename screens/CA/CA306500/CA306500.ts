import {
	createCollection, createSingle, PXScreen, graphInfo, PXView, PXFieldState, gridConfig, PXFieldOptions, PXActionState, columnConfig, handleEvent, CustomEventType, RowSelectedHandlerArgs, PXViewCollection, controlConfig,
	GridColumnDisplayMode, GridPreset
...
export class CA306500 extends PXScreen {
	...
	onDetailsChanged(args: RowSelectedHandlerArgs<PXViewCollection<CABankTran>>) {
		const model = args.viewModel;
		const activeRow = args.viewModel.activeRow;

		if (model.Unhide) model.Unhide.enabled = !!activeRow?.Hidden?.value;
		if (model.Unmatch) model.Unmatch.enabled = !!activeRow?.DocumentMatched?.value;
		if (model.UnmatchAll) model.UnmatchAll.enabled = !!activeRow;
		if (model.ViewDoc) model.ViewDoc.enabled = !!activeRow?.DocumentMatched?.value;
	}
}
