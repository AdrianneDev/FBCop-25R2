import {
	...
	PXScreen, PXActionState, PXView, PXFieldState, PXFieldOptions, headerDescription,
	graphInfo, viewInfo, gridConfig, columnConfig, linkCommand, GridPreset,
	HeaderDescription, GridAutoGrowMode
} from "client-controls";


@graphInfo({ graphType: "PX.Objects.DR.DraftScheduleMaint", primaryView: "Schedule", showActivitiesIndicator: true })
export class DR201500 extends PXScreen {
	...
	@viewInfo({ containerName: "Details -> Components" })
	Components = createCollection(DRScheduleDetail);

	@viewInfo({ containerName: "Details -> Transactions" })
	Transactions = createCollection(DRScheduleTran);

	@viewInfo({ containerName: "Reallocation pool -> Components" })
...
@gridConfig({ autoRepaint: ["Transactions"], preset: GridPreset.Details, adjustPageSize: true })
export class DRScheduleDetail extends PXView {
	...
	@columnConfig({ hideViewLink: true })
	DefCode: PXFieldState<PXFieldOptions.CommitChanges>;

	@columnConfig({ hideViewLink: true })
	DefAcctID: PXFieldState<PXFieldOptions.CommitChanges>;
	...
	TotalAmt: PXFieldState<PXFieldOptions.CommitChanges>;
	DefAmt: PXFieldState;
...
@gridConfig({ preset: GridPreset.ReadOnly, autoRepaint: ["Transactions"], suppressAutoHide: true })
...
@gridConfig({
	...
	parameters: (screen: DR201500) => ({
		"scheduleNbr": screen.Schedule.ScheduleNbr.value.id
	}),
	syncPosition: true,
})
