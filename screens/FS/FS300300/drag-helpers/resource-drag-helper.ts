...
export class ResourceDragHelperConfig {
	...
	addResource?: (resourceId: number | string, eventId: number | string) => Promise<boolean>;
...
export class ResourceDragHelper extends DragHelper {
	...
	protected async onDropFunc({ context, event }) {
		...
		try {
			...
			scheduledOnServer = await this.env.addResource(this.draggedResource.id, hoverAssignment.eventId);
		}
		catch (ex) {
