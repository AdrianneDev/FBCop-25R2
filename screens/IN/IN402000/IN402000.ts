...
export class IN402000 extends PXScreen {
	...
	getAdditionRowCss(args: RowCssHandlerArgs<PXViewCollection<InventoryQtyByPlanType>>) {
	...
	getDeductionRowCss(args: RowCssHandlerArgs<PXViewCollection<InventoryQtyByPlanType2>>) {
		if (args?.selector?.row?.IsTotal.value === true) {
			return "bold-row";
		}
		return undefined;
